		...
		for (int St = 0; St < NS; St++) {
			...
			for (int T = 1; T <= Utils.getLast(S); T++) {
					...
					double Numerator = Operations.multiplyDouble(
							AlphaHelperFunction.Alpha(S, innerKc, T, 1, "fillBest"),
							BetaHelperFunction.Beta(S, innerKc, T, 1, "estimateTransition"));
					double denominatorPart1 = Operations.multiplyDouble(
							AlphaHelperFunction.Alpha(S, innerKc, T, 0, "fillBest"),
							BetaHelperFunction.Beta(S, innerKc, T, 0, "estimateTransition"));
					System.out.println("Alpha(S, innerKc, T, 0) :"
							+ AlphaHelperFunction.Alpha(S, innerKc, T, 0, "fillBest") + " Beta(S,innerKc,T,0)"
							+ BetaHelperFunction.Beta(S, innerKc, T, 0, "estimateTransition"));
					...
					//if (Utils.getKc(1) == innerKc) {
						System.out.println("S :" + S + " innerKc:" + innerKc);
						System.out
								.println("IM :" + Utils.getInitialMasteryMap(innerKc) + "  Slip: " + Utils.getSlipMap(Q)
										+ " Guess :" + Utils.getGuessMap(Q) + "  Lear :" + Utils.getLearnMap(innerKc));
						System.out.println("Answer = " + Utils.getAnswer(S, T) + "  Best[" + T + "] :" + " mastered = "
								+ fillbest + "  not mastered = " + Operations.substractDouble((double) 1, fillbest));
						System.out.println();
					//}
				}
			}
